...
ggplot(data=mydata)+ geom_point(mapping = aes(x=Confirmed, y=Deaths), na.rm = TRUE)

modConfirmed <- lm(data=mydata, formula=Deaths~Confirmed)
summary(modConfirmed)

#Confirmed Cases Predictor (Combination 0)
modConfirmed <- lm(data=mydata, formula=Deaths~Confirmed)
summary(modConfirmed)

#Confirmed 1
modBeds <- lm(data=mydata, formula = Deaths~Confirmed + `Hospital beds (per 10 000 population)`)
summary(modBeds)
...
#Combination 4
urbanratiodata <- mydata %>% mutate(UrbanTotalRatio = `Population (Urban)`/`Population (Total)`)
modUrbanTotal <- lm(data=urbanratiodata, formula = Deaths~Confirmed + UrbanTotalRatio)
...
#Results
myresults <- tibble(model=c("M0", "M1", "M2", "M3", "M4", "M5"), R2 = c(0.89,  0.8885, 0.8894, 0.8952,  0.8912, 0.8888))
myresults

#M0-M5 follow the the myresults tibble names respectively
ggplot(data=myresults) + geom_col(mapping = aes(x=model, y=R2)) + labs(x="Model")
